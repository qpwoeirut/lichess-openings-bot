...
A 8-second `time.sleep` call was added to `lib/lichess-bot.py` to give the user time to update bot settings.
A ThreadPoolExecutor was wrapped around the engine loop so that chat messages can be processed while the engine is thinking.

`.gitignore`, `config.yml.default`, `lib/conversation.py`, `README.md`, and `homemade.py` are the only other edited files.
